...
@testset "Zero-gradient / flat field NaN robustness" begin
    # 1. Curvature zero-gradient test on symmetric 2D grid containing origin (N=31)
    grid = LSM.CartesianGrid((-2.0, -2.0), (2.0, 2.0), (31, 31))
    ϕ = LSM.MeshField(x -> norm(x) - 0.7, grid)
    eq = LSM.LevelSetEquation(;
        terms = (LSM.CurvatureTerm((x, t) -> -0.1),),
        ic = ϕ,
        bc = NeumannBC(),
        integrator = RK2(),
    )
    integrate!(eq, 0.1)
    @test !any(isnan, values(current_state(eq)))

    # 2. EikonalReinitializationTerm zero-gradient flat field test
    grid_1d = LSM.CartesianGrid((-1.0,), (1.0,), (31,))
    ϕ_flat = LSM.MeshField(x -> 0.0, grid_1d)
    eq_flat = LSM.LevelSetEquation(;
        terms = (LSM.EikonalReinitializationTerm(),),
        ic = ϕ_flat,
        bc = NeumannBC(),
        integrator = RK2(),
    )
    integrate!(eq_flat, 0.1)
    @test !any(isnan, values(current_state(eq_flat)))
end
