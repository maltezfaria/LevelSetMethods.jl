...
@testset "Spiral curvature flow — narrow band matches full grid" begin
    ...
    # reinitialize at the start of every step
    ...
    tf = 0.1
    integrate!(eq_full, tf; prehook = reinit_prehook())
    integrate!(eq_nb, tf; posthook = eq -> reinitialize!(current_state(eq)))

    ϕ_full = current_state(eq_full)
    ϕ_nb = current_state(eq_nb)
    max_err = maximum(LSM.active_nodeindices(ϕ_nb)) do I
        abs(ϕ_nb[I] - ϕ_full[I])
    end
    @test max_err < 0.05
end
